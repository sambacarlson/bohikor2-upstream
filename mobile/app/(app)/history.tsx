...
const STATUS_COLORS: Record<string, string> = {
  initiated: "bg-primary-100 text-primary-700",
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <View className={`rounded-full px-3 py-1 ${bg}`}>
      <Text className={`text-sm font-semibold ${text}`}>{status}</Text>
    </View>
  );
}
  ...
  return (
    <View className="bg-white rounded-xl p-5 shadow-sm mb-4">
      ...
      {request.failure_reason && (
        <Text className="text-sm text-red-500 mt-1">{request.failure_reason}</Text>
      )}
    </View>
  );
}
  ...
  if (isLoading) {
    return (
      <View className="flex-1 bg-primary-50 items-center justify-center">
        <ActivityIndicator size="large" color="#7C3AED" testID="history-loading" />
      </View>
    );
  }
    ...
    <View className="flex-1 bg-primary-50">
      ...
      <ScrollView className="flex-1">
        <View className="px-6 pt-2 pb-4 flex-row items-center">
          ...
          <Text className="text-2xl font-bold text-gray-900 flex-1">History</Text>
          <TouchableOpacity onPress={() => refetch()} className="bg-primary-600 rounded-lg px-4 py-2">
